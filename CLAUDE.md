...
- Productive Frogs: **published on CurseForge** (slug `productive-frogs`, project-id `1552728`) as of 2026-05-25. Added via `packwiz cf add productive-frogs`; currently pinned at **`productivefrogs-1.15.0.jar`** (1.21.1 / NeoForge; 2026-06-08 - the Slime Churn [hand-operated milk->slime-bucket block, the Milker run backwards] + **Just Dire Things support** [#188, the PF half of pack #135]: four JDT materials [ferricore Cave, blazegold + celestigem Infernal, eclipsealloy Void] and three Crucible-melted fuels [blaze_ember Infernal, voidflame + eclipse_ember Void] - JDT becomes the 7th Sister Ponds column [33 modded census quests], seven self-keyed chamber rows added. Vanilla roster unchanged, so the Ultimate Singularity holds at **57**). 1.14.0 brought Brewed Froglights [potion auras + Curios slot], four BOSS resources [wither skull/nether star Infernal, dragon egg/breath Void - altar-gated upstream, self-keyed chamber rows pack-side so the real drop is always spent], five mob-drop vanilla stragglers, Mekanism refined_glowstone. The fluid pair water/lava stays excluded - #131, kelp/pointed_dripstone have no item source, exclusion lives in `gen_singularities.py`. The full bump-by-bump history (every version since 1.0.1, what each brought, the 1.8.0 breaking milk split) lives in [`docs/pf_pin_history.md`](./docs/pf_pin_history.md). PF ships no slime-milk-bucket tag, so the pack's `slime_milk_tags.js` stays load-bearing. Pull newer files with `packwiz update productive-frogs`.
