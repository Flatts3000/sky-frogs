...
- Productive Frogs: **published on CurseForge** (slug `productive-frogs`, project-id `1552728`) as of 2026-05-25. Added via `packwiz cf add productive-frogs`; currently pinned at **`productivefrogs-1.14.0.jar`** (1.21.1 / NeoForge; 2026-06-07 - Brewed Froglights [potion auras + Curios slot], four BOSS resources [wither skull/nether star Infernal, dragon egg/breath Void - altar-gated upstream, self-keyed chamber rows pack-side so the real drop is always spent], five mob-drop vanilla stragglers [bone/string/gunpowder/rotten_flesh Bog, magma_cream Infernal], Mekanism refined_glowstone; the Ultimate Singularity went **48 -> 57**. The fluid pair water/lava stays excluded - #131, kelp/pointed_dripstone have no item source, exclusion lives in `gen_singularities.py`). The full bump-by-bump history (every version since 1.0.1, what each brought, the 1.8.0 breaking milk split) lives in [`docs/pf_pin_history.md`](./docs/pf_pin_history.md). PF ships no slime-milk-bucket tag, so the pack's `slime_milk_tags.js` stays load-bearing. Pull newer files with `packwiz update productive-frogs`.
