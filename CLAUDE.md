...
2. **Six categories follow Productive Frogs.** This one is the closest to canonical because it inherits from the mod's enum — but the *progression order* and tier-mapping are open. Settled 2026-05-25: PF v1.0.0's species-as-category redesign means the enum is now BOG / CAVE / GEODE / TIDE / INFERNAL / VOID (biome-species names, player-facing). Sky Frogs adopts these names; the tier mapping is now **settled and built** as Tier 1 = Cave, Tier 2 = Geode, Tier 3 = Bog, with Tier 4 = Tide partially built (the `road_to_tide` Bog -> Tide bridge through the first Prismarine Froglight is live; the full `drowned_riches` species chapter + Tide verb are not yet built; Infernal / Void not started). Two pack-wide design laws settled alongside it: per-tier resource quests check the variant Froglight rather than the smelted resource (so the frog loop can't be bypassed; iron-ingot capstone and the main Mekanism steel quest are the exceptions), and each tier's slime-chain recipe uses a tier-themed filler block (Cave = stone, Geode = gravel, Bog = mossy cobblestone, Tide = mycelium - Tide deliberately breaks the "filler is a plain non-resource block" precedent since mycelium is a Bog chain resource, safe because it's mass-renewable via the Bog loop before Tide).
...
`tools/` helper scripts that exist: `fix_quest_ids.py` (remap negative-leading quest IDs after editing a chapter), `gen_starter_island.py`, `pack_refresh.py` (above), `sync_instance.py` (above). `build_cf_zip.sh` and `slime_variant_codegen.py` are still planned per [`docs/repo_layout.md`](./docs/repo_layout.md) but not yet created.
