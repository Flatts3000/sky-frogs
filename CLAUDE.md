...
- Productive Frogs: **published on CurseForge** (slug `productive-frogs`, project-id `1552728`) as of 2026-05-25. Added via `packwiz cf add productive-frogs`; currently pinned at **`productivefrogs-1.12.0.jar`** (1.21.1 / NeoForge; 2026-06-07 - Froglight Crucible + Casting Mold [the fluids verb], four new vanilla variants [ice/snow/water/lava] taking the Ultimate Singularity 40 -> 44). The full bump-by-bump history (every version since 1.0.1, what each brought, the 1.8.0 breaking milk split) lives in [`docs/pf_pin_history.md`](./docs/pf_pin_history.md). PF ships no slime-milk-bucket tag, so the pack's `slime_milk_tags.js` stays load-bearing. Pull newer files with `packwiz update productive-frogs`.
