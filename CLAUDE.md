...
- Productive Frogs: **published on CurseForge** (slug `productive-frogs`, project-id `1552728`) as of 2026-05-25. Added via `packwiz cf add productive-frogs`; currently pinned at **`productivefrogs-1.13.0.jar`** (1.21.1 / NeoForge; 2026-06-07 - water/lava re-homed to Cave [lava primer now pointed_dripstone] + six vanilla stragglers; with 1.12's Crucible/Mold wave the Ultimate Singularity went 40 -> 50, then settled at **48** when the fluid pair lost its singularities - #131, kelp/pointed_dripstone have no item source, exclusion lives in `gen_singularities.py`). The full bump-by-bump history (every version since 1.0.1, what each brought, the 1.8.0 breaking milk split) lives in [`docs/pf_pin_history.md`](./docs/pf_pin_history.md). PF ships no slime-milk-bucket tag, so the pack's `slime_milk_tags.js` stays load-bearing. Pull newer files with `packwiz update productive-frogs`.
