...
def check_singularity_ingredient(chapters, ctx):
    ...
    Every committed singularity's `ingredient.item` must equal the item its variant's
    Froglight SMELTS to in the PINNED PF jar - the resource the Quantum Compressor is
    supposed to demand. PF 1.11 changed blaze's resource powder -> rod and the
    singularity had to be re-generated by hand-audit; this check makes the next such
    change a validator ERROR instead of a memory test.

    Checked against the smelt recipe, not the variant's `primer_item`: the two agree
    for every vanilla variant but `experience` (primer book, resource experience
    bottle), and asserting the primer is exactly what let the Experience Singularity
    ship demanding books (#245).

    Skips with INFO when no jar is locatable (e.g. CI without the dev instance).
    """
    variants = ctx["pf_variants"]
    resources = ctx["pf_resources"]
    if variants is None or resources is None:
    ...
    for p in sorted(SINGULARITY_DIR.glob("*.json")):
        ...
        if vdata is None:
            ...
            continue
        smelts_to = resources.get(variant, [])
        if len(smelts_to) != 1:
            f.append(Finding(ERROR, "Q-SINGULARITY-INGREDIENT", p.name, 0,
                             f"singularity '{variant}' has {len(smelts_to)} Froglight smelt "
                             f"results in the pinned PF jar ({', '.join(smelts_to) or 'none'}) - "
                             f"expected exactly 1, so there is no single resource to compress. "
                             f"A PF change needs a ruling (exclude the variant, or pick the "
                             f"resource by hand in tools/gen_singularities.py)."))
            continue
        if ingredient != smelts_to[0]:
            f.append(Finding(ERROR, "Q-SINGULARITY-INGREDIENT", p.name, 0,
                             f"singularity '{variant}' ingredient is {ingredient!r} but its "
                             f"Froglight smelts to {smelts_to[0]!r} in the pinned PF jar - stale "
                             f"after a PF resource change (the blaze-rod class). Re-run "
...
# Main
# --------------------------------------------------------------------------- #
def load_pf_variants(explicit_jar: str | None) -> tuple[dict | None, dict | None, list[Finding]]:
    """(variant data, Froglight smelt results, findings about how they were loaded).

    Both come from the same jar, so they share one staleness gate: a jar good enough
    to read variants from is good enough to read recipes from, and a jar rejected as
    stale must not leak into either check.
    ...
    if jar is None:
        return None, None, [Finding(INFO, "Q-PF-JAR", "-", 0,
    ...
    if not explicit_jar:
        ...
        if pinned and actual != pinned:
            return None, None, [Finding(WARN, "Q-PF-JAR", actual, 0,
    ...
    try:
        return pf_jar.load_variants(jar), pf_jar.load_froglight_resources(jar), []
    except Exception as e:
        return None, None, [Finding(WARN, "Q-PF-JAR", Path(jar).name, 0,
...
def run(pf_jar_path: str | None = None, jetpack_cfg: str | None = None):
    ...
    pf_variants, pf_resources, jar_findings = load_pf_variants(pf_jar_path)
