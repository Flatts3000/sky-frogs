...
ServerEvents.recipes(event => {
  ...
  // --- Gap 2: a non-standard ratio the drawer's auto-detect skips (coal) ---
  //
  // Opolis Utilities adds Mini Coal and ships both crafting directions
  // (1 coal -> 8 mini coal; 8 mini coal -> 1 coal). But the drawer's automatic
  // tier detection only understands the standard 4:1 (2x2) and 9:1 (3x3) crafting
  // ratios, so the 8:1 Mini Coal <-> Coal step is invisible to it and a compacting
  // drawer stops at Coal <-> Block of Coal. Functional Storage exposes a
  // `custom_compacting` recipe type for exactly this - it declares a tier directly,
  // ratio and all. We declare both steps so one drawer spans the full three tiers:
  //
