...
- **Honeycomb stacks up in a compacting drawer.** Four honeycomb make a block and the block never uncrafts, so the drawer could not work the pair out on its own. It is spelled out now, the same way Functional Storage already does it for glowstone and quartz.
