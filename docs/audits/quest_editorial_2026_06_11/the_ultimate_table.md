...
| | | | CORRECTED 2026-07-27 (#241): this verdict was WRONG. The catalyst is **not** consumed - `CompressorTileEntity.consumeInputs()` only decrements the material list, and nothing in `tick()`/`process()` touches the catalyst slot. The cited evidence was circular: there is no `defaultCatalyst` key in `extendedcrafting-common.toml`, and the pack README's "+ one Ultimate Catalyst each" was itself the unverified claim. See `docs/known_issues.md`. |
