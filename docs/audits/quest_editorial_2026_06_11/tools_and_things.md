...
|-------|--------------------|-------------------------------------------|---------|
| First Goo | Primogel Goo crafts from clay balls, mycelium, rotten flesh, sugar | `justdirethings data/justdirethings/recipe/gooblock_tier1.json` | WRONG - recipe is `clay_ball` + `dirt` + `rotten_flesh` + `sugar` (pattern `csc/fdf/csc`). Center filler is **dirt**, not mycelium. No mycelium anywhere; the "comes off your Bog mycelium frog" tie-in is fabricated. VERIFY: ~~VERIFIED~~ **REJECTED (2026-06-11, user-caught false positive).** Both the audit and verify agents cracked the JAR JSON (dirt) but missed the pack KubeJS override `kubejs/server_scripts/jdt_recipes.js`, which `event.remove`s `justdirethings:gooblock_tier1` and re-issues it `CSC/FMF/CSC` with center = **mycelium** (deliberately, to gate JDT behind the Bog mycelium frog). The SHIPPING recipe is mycelium; the original quest text ("mycelium," "comes off your Bog mycelium frog") was CORRECT. The "dirt" edit was a regression and has been reverted - First Goo now reads mycelium + "comes off your Bog frog." Lesson: [[project-pf-ships-crossmod-recipes]] - grep pack/kubejs for the recipe id before trusting a jar. |
