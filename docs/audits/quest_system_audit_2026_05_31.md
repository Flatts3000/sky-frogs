...
- ~~**`Q-ITEM-EXISTS` is dormant**~~ **RESOLVED 2026-05-31.** `tools/data/item_ids.txt` (20,317
  ids) is generated by `/sf_dump_items` and committed; the guard is active and clean across all
  123 quests + the reward table (KubeJS 2101.x dropped `/kubejs export`'s item dump, hence the
  custom command).
