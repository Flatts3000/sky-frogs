...
- [ ] **Chapter order + sugar source (DECIDED 2026-05-27).** Sidebar order is Welcome (0) -> Your First Iron Ingot (1) -> **Scaling the Colony (2)** -> **Tier 1: Cave Frogs (3)**. Scaling the Colony comes before Cave Frogs and is where the player gets a **sugar cane** grant (sugar -> `productivefrogs:sweetslime`, PF's frog breeding food). This unblocks both frog breeding and the Cave Frogs **seed-chain**: each Cave resource's "slime in a bucket" recipe takes the prior resource's Slime Milk + slime balls + sweetslime (e.g. copper = iron milk + slime balls + sweetslime), so the chain needs sweetslime and therefore sugar. Cave Frogs' intro ("Beyond Iron") is gated behind Scaling the Colony's **Sweet Tooth** quest (the one that yields sweetslime) - not the Scaling intro - because the Cave chain recipes consume sweetslime, so the player must have it before the chain is reachable. Cave vanilla chain order: **iron (done) -> copper -> gold -> coal -> redstone (end)**. Each step's recipe (`kubejs/server_scripts/cave_slime_chain.js`) is the prior resource's Slime Milk + 4 stone + 3 sweetslime + 1 Cave frogspawn bottle. **Lapis moved to Geode** - fixed in **PF v1.5.1** (Cave -> Geode); pack picks it up on the next `packwiz update productive-frogs` (currently pinned 1.4.3), then lapis joins the Geode tier. **Obsidian** deferred to the Infernal gate. **glow_ink_sac** omitted from the chain (not a stone-and-ore resource).
- [ ] **Modded Cave metals -> their tech-mod chapters (DECIDED 2026-05-27).** The 10 modded Cave-category variants (osmium, tin, lead, aluminum, nickel, silver, zinc, brass, uranium, refined_obsidian) are NOT in the Cave Tier 1 vanilla chain (which ends at redstone). The Cave frog already produces them, so each only needs a one-time bootstrap slime-bucket recipe (cave milk + stone + sweetslime + cave frogspawn -> `<metal>` slime bucket, same pattern as the vanilla chain) - placed in the **tech-mod chapter that needs that metal**. **Osmium** is Mekanism's entry metal and frog-only on a void skyblock, so its bootstrap is the **opening quest of the Mekanism chapter** (this is what resolves "Cave gating Mekanism" - Mekanism stops depending on a missing Cave node and hands the player the osmium path itself). Other tech chapters do the same for their metals.
