...
- [ ] **Produce a 1.21.1 PF build artifact + bundle into Sky Frogs.** PF's gradle targets were updated 2026-05-23 to match Sky Frogs (MC 1.21.1 / NeoForge 21.1.230). Remaining: `./gradlew build` in `../productive-frogs/` to produce the jar, then drop it into `pack/mods/` as a local override (gitignored). The API-rename work captured in the `project_mc1211_api_renames.md` memory will need to be reversed in PF's source; that memory's facts about NeoForge 21.11.42 are now historical PF context, not load-bearing.
