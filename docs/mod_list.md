...
| Mod                   | Role                                                                  |
|-----------------------|------------------------------------------------------------------------|
| **Sodium** (SHIPPED 2026-06-06) | Client rendering performance + better video settings UI. Client-side only. Discord suggestion #73. Pinned on the **0.8.x line** (0.8.12 stable as of v1.5.3): Supplementaries 3.8.4+ declares anything below `0.8.12-beta.1` incompatible, so a revert to the 0.6.x "stable" line crashes the load. |
| **Iris Shaders** (SHIPPED 2026-08-16, 1.8.14-beta.1; client-side) | Loads OptiFine-format shader packs. The pack ships the loader only, not a shader pack - players drop their own into `shaderpacks/`. Discord request from kenny7 and Nferno, tested against the pack by Seeker with Complementary Reimagined. **Pin is coupled to Sodium:** Iris mixins straight into Sodium internals, and 1.8.14-beta.1 is the only 1.21.1 build written against Sodium 0.8 (the 1.8.12 stable is built for 0.6.x). ATM10 runs this same Iris build against Sodium 0.8.12-beta.2; the pack runs 0.8.12 stable, a different jar. **Nothing in either jar's metadata gates the pairing** - 1.8.12 declares an open `sodium [0.6,)` that accepts 0.8 and then dies applying mixins, and 1.8.14-beta.1 declares no sodium dependency at all. So the release-checklist dependency sweep cannot clear a Sodium bump here. Only a launch can. |
