...
| Mod                   | Role                                                                 | Notes                                                |
|-----------------------|----------------------------------------------------------------------|------------------------------------------------------|
| **Skyblock Builder**  | Void worldgen + starting island template + per-player island UI      | 1.21.1. No 1.21.4+ builds exist - partial reason pack is pinned to 1.21.1. |
...
| **Sophisticated Backpacks** + **Sophisticated Storage** + **Sophisticated Core** (SHIPPED) | Upgraded backpacks + barrels. Its controller recipe is re-issued quartz-free in `storage_quartz_free.js` so the analog network is a Cave-era stopgap. |
...
| **Extreme Sound Muffler** (SHIPPED 2026-06-06, per ATM10SKY; client-side) | Per-sound muffling - toggle individual sounds globally or via radius anchors. The frog-farm answer: deafen the plap plaps, keep everything else. Unanimous precedent (all 5 local reference packs ship it). Default config on purpose - muffling stays opt-in for players who like the frogs audible. (#103, suggested by Dergib on Discord) |
...
| **Searchables**, **Controlling**, **Configured** (SHIPPED 2026-06-06, per ATM10SKY; in-game config editing, Discord suggestion #74), **JamLib**, **YACL** | Settings UX                                |
...
| Mod                   | Role                                                                  |
|-----------------------|------------------------------------------------------------------------|
| **Sodium** (SHIPPED 2026-06-06) | Client rendering performance + better video settings UI. Client-side only. Discord suggestion #73. Pinned on the **0.8.x line** (0.8.13 as of v1.8.0): Supplementaries has declared anything below `0.8.12-beta.1` incompatible since 3.8.4, and still does at 3.9.6, so a revert to the 0.6.x "stable" line crashes the load. |
| **Iris Shaders** (SHIPPED 2026-08-16, 1.8.14-beta.1; client-side) | Loads OptiFine-format shader packs. The pack ships the loader only, not a shader pack - players drop their own into `shaderpacks/`. Discord request from kenny7 and Nferno, tested against the pack by Seeker with Complementary Reimagined. **Pin is coupled to Sodium:** Iris mixins straight into Sodium internals, and 1.8.14-beta.1 is the only 1.21.1 build written against Sodium 0.8 (the 1.8.12 stable is built for 0.6.x). ATM10 runs this same Iris build against Sodium 0.8.12-beta.2; the pack runs **0.8.13** as of v1.8.0, a different jar again. **Nothing in either jar's metadata gates the pairing** - 1.8.12 declares an open `sodium [0.6,)` that accepts 0.8 and then dies applying mixins, and 1.8.14-beta.1 declares no sodium dependency at all. So the release-checklist dependency sweep cannot clear a Sodium bump here. Only a launch can - though the v1.8.0 review narrowed the risk without one, by resolving every `net.caffeinemc.mods.sodium.*` reference in Iris 1.8.14-beta.1's 19 `mixins.iris.compat.sodium.json` mixins against Sodium 0.8.13 (nested jars included) at both class and method-name level: zero unresolved. That is a static check, not a render test. |
| **FancyMenu** + **Konkrete** + **Melody** (SHIPPED 2026-06-06, per ATM10SKY; client-side) | Branded title screen (#89): Discord + GitHub buttons live now; wordmark + void panorama slots wired, awaiting art. The frog splash pool ships separately as a vanilla asset override (`kubejs/assets/minecraft/texts/splashes.txt`). |
| **Lithium** (SHIPPED 2026-06-06)          | Server-side performance - game logic optimization (per ATM10SKY)        |
| **FerriteCore** (SHIPPED 2026-06-06)      | Memory savings (per ATM10SKY)                                       |
| **ModernFix** (SHIPPED 2026-06-06)        | Startup + runtime mixins                                              |
