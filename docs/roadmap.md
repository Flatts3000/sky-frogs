...
The pack is **live on CurseForge** (the v0.10.x line), shipping tag-driven releases (client + dedicated-server packs) to CurseForge and GitHub. Productive Frogs is pinned at **1.16.0**. The frog loop and full campaign are built and merged: a couple dozen KubeJS override scripts (anti.js, first_join.js, the per-tier slime chains, the Dissolution Chamber engine + every tier's rows, the spawnery primers, the storage/plastic/steel/log/catalyst/mossy-gate/JDT/boss overrides) and **27 quest chapters / ~329 quests spanning all six tiers** plus the Trophy Pond boss campaign, the Terrarium automation chapter, and the Completionist census. The mod set spans every tier's verb (Mekanism, Industrial Foregoing, Just Dire Things + Building Gadgets, Iron Jetpacks, Refined Storage, Extended Crafting) plus Powah/Flux Networks, the decoration suite, and the QoL stack.
