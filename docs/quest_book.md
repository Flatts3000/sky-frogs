...
| Tier 3: Bog | `tools_and_things.snbt` | The Bog-tier **new verb**: **Just Dire Things' natural tier-1 progression** plus a Building Gadgets 2 side branch. Spine: Primogel Goo Block (intro) -> Ferricore Ingot -> Simple Generator -> Simple Block Breaker -> Item Collector (hexagon capstone). Optionals: Exchanging Gadget -> Copy/Paste Gadget (BG side). |
