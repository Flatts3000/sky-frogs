...
| Tier 3: Bog | `industrial_foregoing.snbt` | The Bog-tier **new verb**: the Industrial Foregoing machine climb (Pity Machine Frame -> Fluid Extractor for latex -> **Dissolution Chamber** capstone, cross-linked to the bog plastic quest), plus an optional Plant Gatherer + Sludge Refiner branch. The Dissolution Chamber is the **Tier 4+ slime engine**: from Tide on, each new tier's slimes are crafted here via resource-threaded recipes (prior tier's last resource -> first new-tier slime; prior variant's resource -> next variant), the same chain shape as the Cave/Geode/Bog crafting-table chains, transposed into the machine. |
