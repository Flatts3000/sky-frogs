...
| Tier 3: Bog | `industrial_foregoing.snbt` | The Bog-tier **new verb**: the Industrial Foregoing machine climb (Pity Machine Frame -> Fluid Extractor for latex -> **Dissolution Chamber** capstone, cross-linked to the bog plastic quest), plus an optional Plant Gatherer + Sludge Refiner branch. The Dissolution Chamber is the **pack's slime engine**: every variant in Cave/Geode/Bog ships with a chamber recipe today (resource-threaded; iron bootstraps off bone meal), and Tier 4+ rows land as those tiers ship. Cave/Geode/Bog crafting-table chains stay as a parallel hand-craft path. |
