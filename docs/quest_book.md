...
| Trophy Pond         | `trophy_pond.snbt` | The **boss campaign** (PF 1.14): two wings + a capstone. Wither wing (gated on the Infernal capstone): fortress skull -> skull farm (chamber or catalyst altar) -> summon the Wither -> nether star farm. Dragon wing (gated on the road_to_void dragon kill): the one Dragon Egg -> the chamber egg LOOP (altar path needs 7 eggs, the world has 1) -> respawn the dragon for breath -> breath farm. Capstone **Apex Pond** wants all four boss Froglights. Teaches the catalyst altar + toxic boss milk. |
