...
| Tier 3: Bog | `tools_and_things.snbt` | The Bog-tier **new verb**: **utility tools** from Just Dire Things + Building Gadgets 2. Spine: Exchanging Gadget -> Copy/Paste Gadget -> Item Collector -> Simple Clicker (hexagon capstone). Optionals: Simple Block Breaker, and a **Dissolution Chamber** side node that introduces the **pack's slime engine** (21 chamber rows for Cave/Geode/Bog ship today, resource-threaded; the chamber moves into `road_to_tide` when Tide ships). Crafting-table chains stay as a parallel hand-craft path. |
...
bog_frogs ------------------+
   |                        |
tools_and_things            | (Bog new verb: utility tools - JDT + BG; chamber lives here as a side node)
   |                        |
- - - - - - - - - - - - - - - - - - - - (sketch below)
   |
