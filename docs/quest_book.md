...
| Tier 3: Bog | `bog_frogs.snbt`             | The Bog species: dirt -> mud -> clay_ball -> moss -> mycelium -> lily_pad -> leather -> feather -> armadillo_scute -> honeycomb -> bone -> gunpowder -> rotten_flesh -> string -> plastic -> pink_slime (plastic hard-gated behind its frog, pink_slime capstone). |
| Tier 3: Bog | `tools_and_things.snbt` | The Bog-tier **new verb**: **Just Dire Things' natural tier-1 progression** plus a Building Gadgets 2 side branch. Spine: Primogel Goo Block (intro; mycelium-gated) -> Ferricore Ingot -> Fluid Collector -> Fluid Placer -> Item Collector (hexagon capstone). Optionals: Exchanging Gadget -> Copy/Paste Gadget (BG side). Mekanism RF from Geode powers the JDT machines. Side branch off the root: **The Slime Churn** (PF 1.15, Bog-craftable from moss + packed mud) - the Milker run backwards, milk -> bucketed slimes with no entity to chase. |
...
| Tier 4: Tide | `road_to_tide.snbt` | Tier 4 gateway. First the Industrial Foregoing spine that ends Bog: Pity Machine Frame -> **A Bucket of Latex** (melt a plastic Froglight in the Crucible) -> **Dissolution Chamber** (hexagon, "The Slime Engine") - the **pack's slime engine** (resource-threaded chamber rows for Cave/Geode/Bog/Tide in `kubejs/server_scripts/dissolution_slime_recipes.js`; iron bootstraps off bone meal). Then the **Bog -> Tide bridge** as two paths off the Bog gate that converge: a **machine path** (Pity Machine Frame -> A Bucket of Latex -> Dissolution Chamber) and a **frog path** (Tide Frogspawn, Spawnery primed with pink_slime), meeting at **A Bucket of Prismarine Slime** (made in the chamber), then a single tail: Milk It -> **First Prismarine** (hexagon capstone). Both paths gate on the bog_frogs pink_slime capstone. **Tide onward is chamber-only** (no crafting-table chain). |
| Tier 4: Tide | `drowned_riches.snbt` | The Tide species chapter: the rest of the aquatic roster, all chamber-made - prismarine_crystals -> sponge -> ink_sac -> sea_pickle -> ice -> snow -> turtle_scute -> nautilus_shell (hexagon capstone). Each quest checks that variant's Froglight (`match_components: strict`). Gates off the road_to_tide First Prismarine capstone. |
...
| Tier 5: Infernal | `infernal_frogs.snbt` | The Infernal species: obsidian -> quartz -> glowstone -> soul_sand -> soul_soil -> blaze -> ghast_tear -> magma_cream -> **netherite_scrap** (hexagon capstone, the vanilla endgame). All chamber-made Froglight checks (`match_components: strict`). Gates off the road_to_infernal First Netherrack capstone. Side branch off the Blaze quest (PF 1.14): **Pop the Cork** + **Charmed, I'm Sure** - the **Brewed Froglight** intro (checkmark/honor-system; the `stored_effect` component is too effect-specific for a strict item task). |
...
| Tier 5: Infernal | `terrarium.snbt` | **The Terrarium** (PF 1.16): the pack's first **automation** of the frog loop. Build the four Infernal-gated blocks (Controller, Sprinklers, Incubators, Hatch) -> Seal the Box (a 5x4x5 shell, machines facing in, sprinklers on the ceiling; right-click the Controller to validate - the in-world build aid) -> Prime the Sprinklers (pipe Slime Milk) -> Stock the Incubators (frogspawn) -> **Hands Off** (hexagon capstone, the loop runs itself). Item tasks for the blocks, checkmarks for assembly/operation (no FTB task detects a formed multiblock). Gated on the infernal_frogs netherite capstone (recipe-faithful, parallel to RS - not locked behind the network). |
...
| Chapter             | File | Covers                                                |
|---------------------|------|--------------------------------------------------------|
| Road to the Void    | `road_to_void.snbt` | The **dragon expedition** gateway: ender pearls (dark-room endermen) -> 2 eyes -> bake the **End Cake** (milk/Slime Milk + egg + wheat + the eyes; each slice teleports to the End, since no stronghold generates) -> Into the End -> **Slay the Dragon** -> the frog sub-chain (frogspawn -> slime -> milk -> First Ender Pearl). Also off the dragon kill: **Princess's Kiss** (PF 1.17) - the dragon's drop turns a frog into a standard villager, a one-Kiss trading-economy bootstrap. |
...
| Trophy Pond         | `trophy_pond.snbt` | The **boss campaign** (PF 1.14): two wings + a capstone, one merged quest per boss (6x the real drop + 6x its catalyst, catalyst-block icon) followed by its Froglight check (resource icon). Wither wing (gated on the Infernal capstone): 6 fortress skulls + skull altar -> Skull Froglight -> 6 Withers + star altar -> Star Froglight. Dragon wing (gated on the road_to_void dragon kill): 6 eggs (**More Dragon Eggs** makes every respawned-dragon kill drop one - vanilla's one-per-world law would make the altar impossible) + egg altar -> Egg Froglight; 6 breath bottles + breath altar -> Breath Froglight. Capstone **The Froglight Cleaver** (PF 1.17, replacing the old Apex Pond checkmark) - forge the endgame sword from two Nether Star Froglights + a Dragon Egg Froglight, wrapped in Dragon's Breath (exactly the trophies the chapter farms). The toxic boss milk (Wither on players) is the chapter's running story beat. |
| Completionist | `whole_pond.snbt`            | **The Whole Pond** (#121, generated by `tools/gen_completionist_chapters.py`): the vanilla census - one froglight quest per vanilla variant (59 today), laid out as a top-down tree frog (head = Cave, body = Bog, four limbs = the other tiers, toes and all), capped by a lily-pad checkmark depending on every one. Deterministic sha-derived ids survive regeneration. |
| Completionist | `sister_ponds.snbt`          | **Sister Ponds** (#121, same generator): the modded census - one column per loaded mod (ATO, Powah, RS, Mekanism, IF, Flux Networks, Just Dire Things; 33 quests across 7 columns today), same capstone pattern. Re-run the generator on every PF pin bump. |
