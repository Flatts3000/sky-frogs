...
- **The Builders' Sieve (#76):** [`builder_sieve.js`](../pack/kubejs/server_scripts/builder_sieve.js) over **dirt** (saplings of every overworld wood, bamboo, sugar cane, cactus, plus grass seeds and the vanilla food-seed lane as of #87/#91) and **moss** (azaleas, vines, glow lichen, dripleaf, every small/tall flower). Cosmetic and building flora plus vanilla food plants. Dirt is composter-cheap at Tier 0; moss is a Bog frog resource. This file also owns the **hardware** - it re-adds the oak sieve and string mesh crafts that `anti.js` strips, and the kitchen lane depends on them.
  - **Sweet berries, glow berries and mushrooms joined in #280.** The audit behind the Kitchen Sieve found these unreachable pack-wide, not just by the food mods. Ex Deorum's own sieve entries are disabled by Pillar 1, no structure generates a berry bush or a mushroom, Farmer's Delight's mushroom colonies are worldgen, and Botany Pots' `sweet_berry_bush` / `brown_mushroom` / `red_mushroom` crops are `block_derived_crop`, so each needs one of itself to plant.
    - **Sweet berries** had no source at all. **Glow berries** had exactly one, Mekanism's combiner, which takes a sweet berry as input. Both use **Ex Deorum's own chances** (sweet berries 0.05 dirt / 0.03 moss, glow berries 0.04, its string-mesh value exactly).
    - **Mushrooms are the exception, and the first version of this note got it wrong.** They were already obtainable: `exdeorum:barrel_fluid_transformation/witch_water` turns a water barrel with a **mycelium** catalyst into witch water over 1700 ticks and drops red and brown mushrooms as 50/50 byproducts, and `anti.js` never touches Ex Deorum's barrels. That path is Bog-tier (mycelium, like moss, is a Bog frog resource), so the moss lane is a **second path at the same tier**, not a first - justified because the barrel route is undiscoverable without reading the mod's data, and **#270 proposes removing the witch water recipes**, which would take the only mushroom source with them. Ex Deorum ships no mushroom *sieve* recipe, so 0.05 is a pack decision.
