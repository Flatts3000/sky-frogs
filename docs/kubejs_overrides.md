...
- [`dissolution_slime_recipes.js`](../pack/kubejs/server_scripts/dissolution_slime_recipes.js). **Currently disabled** (registers nothing). Intended: Industrial Foregoing Dissolution Chamber recipes that make per-variant slimes. Blocked because IF/Titanium sync recipes as JSON, and the `neoforge:components` ingredient needed to select a variant by its milk component does not survive that round-trip (it crashes `clientbound/minecraft:update_recipes`). Deferred pending a PF feature that exposes a distinct-per-variant item handle ([productive-frogs#127](https://github.com/Flatts3000/productive-frogs/issues/127)); slime production stays on the crafting-table chains until then.
