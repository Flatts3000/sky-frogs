...
- [`dissolution_slime_recipes.js`](../pack/kubejs/server_scripts/dissolution_slime_recipes.js). Industrial Foregoing **Dissolution Chamber** recipes that make slimes: for every unlocked Cave/Geode/Bog variant, 100 mb latex + that variant's Slime Milk (the variant *selector* via a `neoforge:components` ingredient - PF's milk fluid is variant-agnostic, so the variant rides on the bucket item) + 4 tier-filler + 3 sweetslime -> that variant's Slime in a Bucket. The chamber is gated at end-of-Bog (its frame needs plastic), so this is the IF-era production path; from Tier 4 on the same machine also becomes the seed-chain bootstrap (one table entry per tier).
