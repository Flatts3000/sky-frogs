...
| **Q-SINGULARITY-INGREDIENT** | ERROR (INFO-skip without the jar) | every committed singularity's `ingredient.item` equals its variant's `primer_item` in the **pinned PF jar** (looked up in the dev instance, or `--pf-jar PATH`) | the #101 / blaze-rod class: PF 1.11 changed blaze powder -> rod and the singularity went stale until a hand-audit caught it. Now a validator ERROR |
