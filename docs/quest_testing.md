...
> its allowlist `tools/data/item_ids.txt` (20,317 ids) is generated by `/sf_dump_items`
...
| **Q-DISSOLUTION-COVERAGE** | ERROR (INFO-skip without the jar) | every modded variant the pack EXPOSES (owning mod in `LOADED_MODS`, computed by reusing `gen_completionist_chapters.py`'s `variant_mod` so "exposed" == what Sister Ponds emits) has a NON-circular slime source: a dissolution row (self-keyed or a tier-chain step) or a bespoke bucket/infusing recipe. The generated Froglight smelt-back does not count | the #167 / silicon class: the hand-maintained dissolution list and the GENERATED Froglight recipes + census were unreconciled, so silicon shipped with a Froglight recipe and a census quest but no dissolution row - its only slime path was the circular smelt-back |
