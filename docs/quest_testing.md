...
> (`/sf_selftest`, Layer 2 runtime canaries). All checks are now active, including `Q-ITEM-EXISTS`:
> its allowlist `tools/data/item_ids.txt` (21,469 ids) is generated by `/sf_dump_items`
> (`pack/kubejs/server_scripts/dump_items.js`) since KubeJS 2101.x dropped the old `/kubejs export`
> item dump. Regenerate it after any mod-list change.
...
| **Q-SINGULARITY-INGREDIENT** | ERROR (INFO-skip without the jar) | every committed singularity's `ingredient.item` equals the item its variant's Froglight **smelts to** in the **pinned PF jar** (looked up in the dev instance, or `--pf-jar PATH`), and that variant has exactly one smelt result | the #101 / blaze-rod class: PF 1.11 changed blaze powder -> rod and the singularity went stale until a hand-audit caught it. Now a validator ERROR. Checked against the smelt recipe, not `primer_item`: the two diverge for `experience` (primer book, resource experience bottle), and asserting the primer is what let #245 ship |
| **Q-DISSOLUTION-THREADING** | ERROR (INFO-skip without the jar) | `dissolution_slime_recipes.js` diffed against the jar: every `SLIME_TIERS` row's input == the prior variant's `primer_item` (tier boundaries bridge off the prior tier's last; `iron <- bone_meal` is the documented exception), and every `MODDED_SELF_KEYED` row's input == that variant's OWN primer (the make-it-first law, PR #106; tag-primed variants expect `#<tag>`, and the water/lava fluid pair are documented bucket-keyed exceptions) | the other half of #101: a PF resource change that updates the singularity but misses the chamber row (or vice versa) desyncs the engine from the endgame |
| **Q-DISSOLUTION-COVERAGE** | ERROR (INFO-skip without the jar) | every variant the pack ships IN GAME (no `mod_loaded` condition at all, or one naming a mod in `LOADED_MODS`) has a NON-circular slime source: a dissolution row (self-keyed or a tier-chain step) or a bespoke bucket/infusing recipe. The generated Froglight smelt-back does not count | the #167 / silicon class: the hand-maintained dissolution list and the GENERATED Froglight recipes + census were unreconciled, so silicon shipped with a Froglight recipe and a census quest but no dissolution row - its only slime path was the circular smelt-back. **Widened by #286**, which is why it keys on the mod condition rather than asking the census generator's question: it used to test `not is_vanilla` AND owning mod in `LOADED_MODS`, and PF 1.26.0's `rainbow` is neither (no `minecraft:` primer_item, no condition), so it fell out of the check's INPUT and the check passed by never considering it. Vanilla variants are in scope now - most are covered by their `SLIME_TIERS` chain row |
| **Q-TABLE-CHAIN-MIRROR** | ERROR | every `[from, to]` step in `cave/geode/bog_slime_chain.js` is an ADJACENT pair in the same tier's `SLIME_TIERS` block (or the bridge from the prior tier's last variant); no jar needed (pack file vs pack file) | the #125 breeze-slime gap: a variant added to the chamber chain but forgotten in the table chain ships with a chamber recipe and no crafting recipe |
| **Q-ATO-CHAIN-MIRROR** | ERROR | `ato_slime_chain.js` forms ONE connected chain rooted at osmium, and its outputs exactly match the chamber's `alltheores` self-keyed rows minus the root (which must still hold a row); no jar needed | the #125 class transplanted to the modded table (PR #126 review): a new ATO metal added to one surface but not the other ships a census quest with no bootstrap, or a chain step with no scaling path |

| **Q-JETPACK-COIL** | ERROR (INFO-skip without the config) | every claim the Take Flight copy makes about Iron Jetpacks holds against the live tier ladder: which coil each named jetpack takes, that `wood` is the only from-scratch build, and that iron upgrades from stone. Computed by `tools/ironjetpacks_tiers.py` from the dev instance's `config/ironjetpacks/jetpacks/` plus any pack-shipped override (or `--jetpack-config PATH`) | the #233 class: Iron Jetpacks picks a coil from the jetpack's *position* in the registered-tier list (`index/count` against 0.25/0.5/0.75) and gives a from-scratch recipe to the lowest tier ONLY. Nothing in this repo shows either fact, so the chapter shipped telling players to craft an Iron Jetpack from a Basic Coil - a recipe that does not exist - until a player reported it. Adding or disabling any jetpack re-maps the ratio and makes the copy wrong again |
...
then `/sf_dump_items` (registered by `pack/kubejs/server_scripts/dump_items.js`) writes the full
`Item.getTypeList()` to `logs/kubejs/server.log` in `SF_ITEM_DUMP::` chunks; extract those into the
file. (KubeJS 2101.x removed the old `/kubejs export` item dump - its `export` subcommands are only
debug / pack-folders / pack-zips / recipe-schema-json.) If the file is absent, `Q-ITEM-EXISTS` is
...
| **2** | cross-checks (Q-VARIANT-MADE DONE; Q-ITEM-EXISTS **DONE** - active against the 20,317-id `tools/data/item_ids.txt`; Q-FROGLIGHT-IS-CHECK deferred - too heuristic to define without false positives) | "Missing Item" gaps, quest/recipe drift | **DONE** (bar the deferred froglight heuristic) |
...
`match_components` or a dangling dependency makes it exit non-zero with the exact finding.
`Q-ITEM-EXISTS` is now active against `tools/data/item_ids.txt` (regenerate via `/sf_dump_items`
after a mod-list change, see the check note above).
