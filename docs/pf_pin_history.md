...
- When a bump changes item ids, recipes, or variant categories, run `tools/sync_instance.py` (so the new jar is local) then `tools/validate_quests.py` - the three-places audit is now **automated** (#101): Q-SINGULARITY-INGREDIENT diffs every singularity against the jar's `primer_item`, Q-DISSOLUTION-THREADING diffs the chamber rows (chain contiguity + the self-keyed law), and Q-SINGULARITY-COVERAGE / Q-VARIANT-MADE cover the coverage classes. The remaining MANUAL surface is quest LANG text (prose can't be diffed against a jar) and the crafting-table chain scripts.
