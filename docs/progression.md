...
6. **One new verb per tier (anti-repetition).** Each tier layers exactly one new verb onto the frog loop so the gameplay doesn't just repeat at a higher number:
   ...
   - **Bog** = Just Dire Things' tier-1 progression (Primogel Goo -> Ferricore -> Simple Generator -> Simple Block Breaker -> Item Collector) plus a Building Gadgets 2 side branch. Lives in `tools_and_things.snbt`.
...
- **New verb:** **Just Dire Things tier-1 progression** + Building Gadgets 2 side branch (the `tools_and_things.snbt` chapter). Spine: Primogel Goo Block -> Ferricore Ingot -> Simple Generator -> Simple Block Breaker -> Item Collector (hexagon capstone). Optionals: Exchanging Gadget -> Copy/Paste Gadget.
