...
2. If the pin is several patches behind, bump it to the latest 21.1.x (edit `pack/pack.toml` + the references in `CLAUDE.md` / `docs/pack_metadata.md` / `docs/cf_submission_checklist.md`), then run **`python tools/sync_instance_loader.py`** and **launch-test the dev instance on the new loader** before shipping. The sync step is not optional: `sync_instance.py` mirrors jars only, so without it the instance stays on its old loader and the "launch test" either fails for a reason that has nothing to do with the pack, or passes on a loader you are not shipping. v1.5.3 shipped without this test because the instance was still on 21.1.230 while the pack pinned 21.1.244.
